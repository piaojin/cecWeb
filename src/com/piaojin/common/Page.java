...
public class Page<E> extends AbstractList<E> implements List<E>, RandomAccess, Cloneable, java.io.Serializable {
    ...
    private int pageNum;  //
    private int numPerPage;  //姣忛〉鏄剧ず澶氬皯锟�
    private int totalCount;  //鎬绘潯锟�
    ...
    public Page(Collection<? extends E> c) {
        size = c.size();
        // Allow 10% room for growth	//10%浣滀负澧為噺
     ...
     * 褰撳璞℃暟缁勫ぇ浜庡璞＄被闆嗘椂锛屾牴鎹璞＄被闆嗙缉鏀惧璞℃暟缁勶紙鎶婂璞℃暟缁勫噺鑷虫渶浣庯級
     */
    public void trimToSize() {
    modCount++;
    int oldCapacity = elementData.length;	//瀵硅薄鏁扮粍 澶у皬
    if (size < oldCapacity) {
    ...
    modCount++;
    int oldCapacity = elementData.length;	//瀵硅薄鏁扮粍
    if (minCapacity > oldCapacity) {		//鏂版暟缁勫昂瀵稿ぇ浜庡璞℃暟缁勫昂锟�
        Object oldData[] = elementData;				
        int newCapacity = (oldCapacity * 3)/2 + 1;	//澧炲锛岀畻娉曪細1.5*capacity+1
        if (newCapacity < minCapacity)
        newCapacity = minCapacity;
        elementData = (E[])new Object[newCapacity];
        System.arraycopy(oldData, 0, elementData, 0, size);	//鎵ц鎷疯礉
    }
    }
    ...
    Object[] a = c.toArray();
        int numNew = a.length;
    ensureCapacity(size + numNew);  // Increments modCount澧炲
